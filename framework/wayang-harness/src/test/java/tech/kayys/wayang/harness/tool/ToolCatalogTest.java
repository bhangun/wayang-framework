...
class ToolCatalogTest {
    ...
    void setUp() {
        ...
        dummyProvider = new ToolProvider() {
            ...
            public Optional<ToolDescriptor> describe(ToolId id) {
                return id.equals(toolDescriptor.id()) ? Optional.of(toolDescriptor) : Optional.empty();
            }
            ...
            public ToolExecutor executor(ToolId id) {
                return (inv, ctx) -> ToolResult.success(inv.id(), "ok", java.time.Duration.ZERO, "dummy");
            }
        };
    }
    ...
    void shouldGetToolDirectlyById() {
        ...
        assertThat(desc.get().capabilities()).contains("filesystem.read");
